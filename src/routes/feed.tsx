...
import { Flame, Eye, MessageCircle, Instagram, Heart, Copy, EyeOff, Shuffle } from "lucide-react";
...
export const Route = createFileRoute("/feed")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Casos de éxito reales de la comunidad. Copiá las técnicas y aplicalas.",
      },
    ],
  }),
  component: Feed,
...
type Post = {
  ...
  views: number;
  chat: string;
  technique: { name: string; how: string };
};
...
const POSTS: Post[] = [
  {
    ...
    technique: {
      name: "Observación + cumplido indirecto",
      how: "Fijate en UN detalle de la foto y conectalo con algo tuyo o un juego de palabras. No es halago plano, es un guiño.",
    },
  },
  {
    user: "Diego",
    age: 27,
    vibe: "Tranquilo · empático",
    platform: "WhatsApp",
    ...
    technique: {
      name: "Callback específico",
      how: "Rescatá un detalle puntual que ella mencionó al pasar. Demuestra atención sin parecer obsesivo.",
    },
  },
  {
    user: "Tomás",
    age: 22,
    vibe: "Frame fuerte · juguetón",
    platform: "Tinder",
    ...
    technique: {
      name: "Mantener frame ante shit tests",
      how: "Cuando te testea, NO te justifiques. Respondé con humor que reframea: 'Mi modestia. Después te muestro otras virtudes.'",
    },
  },
  {
    user: "Joaco",
    age: 29,
    vibe: "Romántico · old school",
    platform: "Bumble",
    ...
    technique: {
      name: "Silencio estratégico en el cierre",
      how: "Después de la actividad principal, llevala a un lugar tranquilo y NO llenes el silencio. El espacio crea tensión.",
    },
  },
  {
    user: "Bruno",
    age: 31,
    vibe: "Maduro · misterioso",
    platform: "Instagram",
    ...
    technique: {
      name: "Ratio inverso",
      how: "Bajá la frecuencia. La gente desea lo que no puede tener todos los días.",
    },
  },
  {
    user: "Lucas",
    age: 26,
    vibe: "Atrevido · juguetón",
    platform: "TikTok",
    ...
    technique: {
      name: "Cold-read específico",
      how: "Tirá una observación arriesgada pero plausible. Si pegás, sos un mago. Si no, sos un loco gracioso.",
    },
  },
  {
    user: "Nacho",
    age: 23,
    vibe: "Tímido en remisión",
    platform: "WhatsApp",
    ...
    technique: {
      name: "Honestidad de alto frame",
      how: "Decir la verdad con calma proyecta más confianza que cualquier técnica. Pero practicala antes.",
    },
  },
  {
    user: "Pablo",
    age: 28,
    vibe: "Sarcástico · culto",
    platform: "Bumble",
    ...
    technique: {
      name: "Frame alto desde el opener",
      how: "El primer mensaje setea la dinámica. Empezá ya como premio.",
    },
  },
];
...
const ALL_PLATFORMS: ("Todas" | Platform)[] = [
  "Todas",
  "Instagram",
  "Tinder",
  "Bumble",
        ...
        <div className="flex-1 flex gap-1.5 overflow-x-auto pb-1 scrollbar-none">
          {ALL_PLATFORMS.map((p) => (
            <button
              key={p}
              onClick={() => setFilter(p)}
              className={`chip whitespace-nowrap ${filter === p ? "chip-active" : ""}`}
            >
              {p}
            </button>
          ))}
...
function Card({ user, age, vibe, platform, text, fires, views, chat, technique }: Post) {
    ...
    <article className="neon-card rounded-2xl p-4">
      ...
      <div className="mt-3 rounded-xl border border-[rgba(168,85,247,0.2)] bg-[rgba(15,25,55,0.55)] p-3 relative">
        ...
        >
          {revealChat ? (
            <>
              <EyeOff className="h-3 w-3" /> Ocultar chat
            </>
          ) : (
            <>
      ...
      {/* Técnica */}
      <div className="mt-3">
        <button onClick={() => setShowTech((v) => !v)} className="btn-ghost w-full !py-2.5">
      ...
      <div className="flex items-center gap-4 mt-3 text-xs text-[#BFDBFE]/70">
        ...
        <span className="flex items-center gap-1.5">
          <MessageCircle className="h-3.5 w-3.5" /> {Math.floor(views / 30)}
        </span>
      </div>
    </article>
  );
