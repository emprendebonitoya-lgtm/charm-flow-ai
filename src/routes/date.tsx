...
export const Route = createFileRoute("/date")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Planificá una cita en 3 fases: Apertura, Conexión y Cierre." },
    ],
  }),
  ...
  const run = async () => {
    ...
    try {
      const res = await chat({
        data: {
          messages: [
            { role: "system", content: "Sos un planificador de citas. Devolvé EXACTAMENTE un JSON: array con 3 objetos {fase, titulo, detalle}. Las fases son: 'Apertura', 'Conexión', 'Cierre'. Detalle ≤180 chars en español neutro masculino, concreto." },
            { role: "user", content: `Intereses: ${interest.join(", ")}. Presupuesto: ${budget}. Vibra: ${vibe}.` },
          ],
          temperature: 0.8,
        },
      });
    ...
    <AppShell title="Date Planner" subtitle="3 fases. 0 improvisación.">
      <div className="space-y-4">
        <div className="neon-card rounded-2xl p-4">
          <div className="text-[10px] uppercase tracking-[0.28em] text-muted-foreground mb-2">Intereses de ella</div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="neon-card rounded-2xl p-4">
            <div className="text-[10px] uppercase tracking-[0.28em] text-muted-foreground mb-2">Presupuesto</div>
            <div className="flex gap-2">
              {BUDGET.map((b) => (
                <button key={b} onClick={() => setBudget(b)}
                  className={`flex-1 px-2 py-1.5 rounded-xl text-xs border ${budget === b ? "grad-cyber text-[#04060a] border-transparent" : "border-[rgba(0,240,255,0.18)] text-muted-foreground"}`}>{b}</button>
              ))}
            </div>
          </div>
          <div className="neon-card rounded-2xl p-4">
            <div className="text-[10px] uppercase tracking-[0.28em] text-muted-foreground mb-2">Vibra</div>
            <div className="grid grid-cols-2 gap-1.5">
              {VIBES.map((v) => (
                <button key={v} onClick={() => setVibe(v)}
                  className={`px-2 py-1.5 rounded-xl text-xs border ${vibe === v ? "grad-cyber text-[#04060a] border-transparent" : "border-[rgba(0,240,255,0.18)] text-muted-foreground"}`}>{v}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        {plan && (
          <div className="relative pl-8 mt-4 animate-fade-in">
            <div className="absolute left-3 top-2 bottom-2 w-px bg-gradient-to-b from-[#00F0FF] via-[#0047FF] to-[#4D00FF]"
            ...
            {plan.map((p, i) => {
              ...
              return (
                <div key={i} className="relative mb-4">
                  ...
                  <div className="neon-card rounded-2xl p-4">
                    <div className="text-[10px] uppercase tracking-[0.28em] text-[#00F0FF]">{p.fase}</div>
                    <div className="font-display text-base mt-1">{p.titulo}</div>
                    <p className="text-sm text-muted-foreground mt-1.5 leading-relaxed">{p.detalle}</p>
                  </div>
                </div>
              );
            })}
