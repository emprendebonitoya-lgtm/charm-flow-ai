...
const LEGACY_HISTORY_KEY = "rizz_history_v1";
const LEGACY_SAVED_KEY = "rizz_saved_v1";
const LEGACY_PROFILE_KEY = "rizz_profile_v1";
const HISTORY_KEY = "magneto_history_v1";
const SAVED_KEY = "magneto_saved_v1";
const PROFILE_KEY = "magneto_profile_v1";
...
function readWithMigration<T>(key: string, legacyKey: string, fallback: T): T {
  const current = read<T>(key, fallback);
  if (typeof window === "undefined") return current;
  const hasCurrent = localStorage.getItem(key) !== null;
  if (hasCurrent) return current;
  const legacy = read<T>(legacyKey, fallback);
...
export const getSaved = () => readWithMigration<HistoryItem[]>(SAVED_KEY, LEGACY_SAVED_KEY, []);
